    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {pending && (
        <View style={styles.pay}>
          ...
          <View style={styles.chips}>
            {METHODS.map((m) => (
              <Text key={m.key} onPress={() => setMethod(m.key)} style={[styles.chip, method === m.key && styles.chipActive]}>
                {m.label}
              </Text>
            ))}
          </View>
          ...
          <View style={styles.chips}>
            <Text onPress={() => setSimulate("approve")} style={[styles.chip, simulate === "approve" && styles.chipActive]}>
              simular: aprovar
            </Text>
            <Text onPress={() => setSimulate("decline")} style={[styles.chip, simulate === "decline" && styles.chipActive]}>
              simular: recusar
            </Text>
          </View>
          ...
          <Button label={pay.isPending ? "Processando…" : "Pagar"} onPress={() => pay.mutate({ id: order.id, method, simulate })} disabled={pay.isPending} />
      ...
      {timeline && timeline.length > 0 && (
        <View style={styles.timeline}>
          ...
          {/* Tipagem explícita para o parâmetro `t` */}
