...
//   const { removeFavorite, isRemoving } = useFavoriteMutations();
...
//         refreshControl={<RefreshControl refreshing={isFetching && !isLoading} onRefresh={() => refetch()} tintColor="#dc2626" />}
//         ListEmptyComponent={<Text style={styles.empty}>Você não tem peças salvas nos favoritos.</Text>}
//         renderItem={({ item }) => (
//           <View style={styles.card}>
//             <Pressable style={styles.cardInfo} onPress={() => navigation.navigate("ProductDetail", { id: item.productId, name: item.productName })}>
//               {item.image ? <Image source={{ uri: item.image }} style={styles.thumb} /> : <View style={[styles.thumb, styles.thumbEmpty]} />}
...
//               </View>
//             </Pressable>
//             <Button label="Remover" variant="ghost" disabled={isRemoving} onPress={() => removeFavorite(item.variantId)} />
//           </View>
//         )}
//       />
//     </View>
          ...
          return (
            <View style={styles.card}>
              ...
              <Button
                ...
                variant="ghost"
                disabled={isRemoving}
                onPress={() => handleRemove(item.variantId || item.id, displayTitle)}
              />
            </View>
          );
        }}
